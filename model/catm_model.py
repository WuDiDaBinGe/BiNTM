...
from model.gan import ContrastiveDiscriminator
...
class CATM(BNTM):
    ...
    def train_with_contra(self, train_data, gamma_temperature, gamma_cluster_temperature, batch_size=512, clip=0.01,
...
class CBTM(BNTM):
    ...
    def __init__(self, n_topic, bow_dim, hid_dim, device, task_name):
        ...
        self.date = time.strftime("%Y-%m-%d-%H-%M", time.localtime())
        self.discriminator = ContrastiveDiscriminator(n_topic=n_topic, v_dim=bow_dim, hid_features_dim=hid_dim / 2)
        if device is not None:
    ...
    def train_with_contra(self, train_data, gamma_temperature, gamma_cluster_temperature, batch_size=512, clip=0.01,
        ...
        if resume and ckpt_path is not None:
            # TODO: 断点续训的时候需要改路径
            self.writer = SummaryWriter('log/c_atm_discriminator/20news_clean_2021-07-12-22-03_topic20')
            checkpoint = torch.load(ckpt_path)
            self.discriminator.load_state_dict(checkpoint['discriminator'])
            self.generator.load_state_dict(checkpoint['generator'])
            self.encoder.load_state_dict(checkpoint['encoder'])
        ...
        for epoch in range(start_epoch + 1, epochs + 1):
            for iter_num, data in enumerate(data_loader):
                ...
                # use detach() to stop the gradient backward p(frozen the g and e parameters)
                # advertise loss
                loss_D = -torch.mean(self.discriminator(self.encoder(bows_real).detach(), bows_real)) + torch.mean(
                    self.discriminator(topic_fake, self.generator(topic_fake).detach()))
                # contrastive loss
                # contrastive_loss_real =
                loss_D.backward()
                optim_d.step()
                # Clip weights of discriminator
                for p in self.discriminator.parameters():
                ...
                if iter_num % n_critic == 0:
                    ...
                    z_i = F.normalize(self.instance_head(h_i), dim=1)
                    z_j = F.normalize(self.instance_head(h_j), dim=1)
                    instance_loss = contrastive_loss_function(z_i, z_j)
                    loss_e = torch.mean(self.discriminator(p_real))

                    loss_E = loss_e + 0.1 * instance_loss
                    loss_E.backward()
                    optim_e.step()
                    print(
                        f'Epoch {(epoch + 1):>3d}\tIter {(iter_num + 1):>4d}\tLoss_D:{loss_D.item():<.7f}\tLoss_G:{loss_G.item():<.7f}\tloss_E:{loss_e.item():<.7f}\tcluster_loss:{cluster_loss}\tinstance_loss:{instance_loss.item():<.7f}')
